...
## ⚠️【最重要】アイコン追加・変更時のチェックリスト

アプリのアイコン（Lucide Icon）を新規設定・変更する場合は、**必ず以下の4箇所すべて**に変更を反映してください。一つでも漏れると「雷マーク（zap）になってしまう」「faviconだけ古いままになる」等の不整合が発生します。

- [ ] **1. `meta.json`**: `"icon"` フィールドにアイコン名を指定する。
- [ ] **2. `favicon.svg`**: アプリディレクトリ内に SVG を配置し、線の色を `meta.json` の `"color"` と同じ HEX カラーにする。
- [ ] **3. `shared/header.js`**: `ICONS` オブジェクトに SVG の `<path>` 等を追加する。
- [ ] **4. `hub/script.js`**: `ICON` オブジェクトに SVG の `<path>` 等を追加する。

> **AIアシスタントへの指示**: このミスは非常に発生しやすいため、アイコンを変更するタスクを行った際は、必ず上記4ファイルに変更が適用されているかを最終確認すること。
