...
test('index.html は共通 CSS / 共通ヘッダー / モジュール script を読み込む', () => {
  ...
  assert.ok(indexHtml.includes('data-app-icon="mic"'));
  assert.ok(indexHtml.includes(`data-app-color="${meta.color}"`));
});

test('同意画面はモデルサイズを明示する', () => {
...
test('worker.js は Transformers.js の GraniteSpeech を pipeline.mjs 経由で使う', () => {
  assert.ok(workerJs.includes("from './pipeline.mjs'"));
  assert.ok(workerJs.includes('GraniteSpeechForConditionalGeneration'));
  assert.ok(workerJs.includes('AutoProcessor'));
  assert.ok(workerJs.includes('TextStreamer'));
  assert.ok(workerJs.includes('DTYPE_WEBGPU'));
  assert.ok(workerJs.includes('DTYPE_WASM'));
...
test('script.js は init / transcribe / cancel / clear-cache を worker に送る', () => {
  for (const type of ['init', 'transcribe', 'cancel', 'clear-cache']) {
    assert.ok(scriptJs.includes(`type: '${type}'`), `${type} を送っていない`);
  }
});

test('script.js は録音・ドロップ・自動分割に対応する', () => {
  ...
  assert.ok(scriptJs.includes('findSpeechSegments'));
  assert.ok(scriptJs.includes('computePeaks'));
});
