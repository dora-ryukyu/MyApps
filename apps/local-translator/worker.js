...
async function translate({ text, direction, requestId }) {
  if (busy) {
    post('error', { stage: 'process', requestId, error: '別の翻訳を処理中です。' });
    return;
  }
  if (!model || !tokenizer) {
  ...
  try {
    ...
    post('result', {
      requestId,
      text: cleanTranslation(resultText),
      tokens: tokenCount,
