...
test('index.html は共通 CSS / 共通ヘッダー / モジュール script を読み込む', () => {
  ...
  assert.ok(indexHtml.includes('data-app-icon="eraser"'));
...
test('worker.js は background-removal パイプラインを使う', () => {
  assert.ok(workerJs.includes("from './pipeline.mjs'"));
  assert.ok(workerJs.includes("'background-removal'"));
...
test('worker.js は RMBG-1.4 を Segformer クラス明示でフォールバックする', () => {
  // Transformers.js の AutoModel 解決では RMBG-1.4 を読み込めないため、
  // アーキテクチャクラスを直接使う経路を持つ。
  assert.ok(workerJs.includes('SegformerForSemanticSegmentation'));
  assert.ok(workerJs.includes('AutoProcessor'));
  assert.ok(workerJs.includes('putAlpha'));
  assert.ok(workerJs.includes('sigmoid_'));
});
...
test('script.js は画像入力・合成・保存・モデル選択の純関数を使う', () => {
  ...
  assert.ok(scriptJs.includes('compose('));
...
test('registry.json (生成物) があれば bg-remove を含む', (t) => {
  ...
  assert.ok(registry.apps.includes('bg-remove'), 'registry.json に bg-remove が無い');
